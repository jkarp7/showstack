...
export function registerInfrastructureHandlers(): void {
  ...
  ipcMain.handle(
    ...
    async (_event, equipment: Partial<InfrastructureEquipment>, projectId: string) => {
      try {
  ...
  ipcMain.handle(
    ...
    async (_event, id: string, updates: Partial<InfrastructureEquipment>) => {
      try {
        // Validate with Zod schema
  ...
  ipcMain.handle(
    ...
    async (_event, equipmentId: string, projectId: string) => {
      try {
  ...
  ipcMain.handle(
    ...
    async (_event, fixtureId: string, projectId: string) => {
      try {
        return await errorHandler.executeWithRetry(
  ...
  ipcMain.handle(
    ...
    async (_event, equipmentId: string, portAssignment: PortAssignment, projectId: string) => {
      try {
        return await errorHandler.executeWithRetry(
  ...
  ipcMain.handle(
    ...
    async (_event, projectId: string, csvFilePath: string, fieldMapping: CSVFieldMapping[]) => {
      try {
        return await errorHandler.executeWithRetry(async () => {
  ...
  ipcMain.handle('infrastructure:showImportDialog', async () => {
    try {
      ...
      throw new Error(
        `Unable to show import dialog: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
    }
  });
