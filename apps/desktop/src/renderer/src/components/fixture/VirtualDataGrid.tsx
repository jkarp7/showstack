...
export function VirtualDataGrid({
  ...
  const handleCellEdit = useCallback(
    (fixtureId: string, field: keyof Fixture, value: string) => {
        ...
        if (containerRef.current) {
          ...
          if (rowY < containerTop) {
            ...
            containerRef.current.scrollTop =
              rowY + ROW_HEIGHT - containerRef.current.clientHeight + HEADER_HEIGHT;
          }
        }
      }
    },
  ...
  const handleDrop = useCallback(
    (e: React.DragEvent, targetColumnKey: ColumnKey) => {
      e.preventDefault();
  ...
  const handleResizeStart = useCallback(
    (e: React.MouseEvent, columnKey: ColumnKey, currentWidth: number) => {
      e.preventDefault();
      e.stopPropagation();
  ...
  const getColumnWidth = useCallback(
    (col: (typeof COLUMN_CONFIGS)[0]): number => {
      ...
      >
        <div className="w-12 flex items-center justify-center flex-shrink-0">
          <input
            ...
            style={{
              backgroundImage:
                selectedRows.size === fixtures.length && fixtures.length > 0
