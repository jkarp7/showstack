...
import {
  HighlightRule,
  getHighlightColorForFixture,
  COLOR_FLAG_DEFINITIONS,
  ColorFlagType,
...
export const VirtualRow = memo(function VirtualRow({
  ...
  const getCellValue = (key: ColumnKey): string => {
    ...
    if (
      (key === 'changed_at' || key === 'created_at' || key === 'updated_at') &&
      typeof value === 'number'
    ) {
  ...
  return (
    <div
      className={`flex items-center h-10 border-b border-gray-200 dark:border-gray-800 relative ${
      ...
      <div className="flex items-center flex-1 h-full pl-[2px]" style={rowStyle}>
        ...
        {orderedColumns
          ...
          .map((col) => {
            ...
            const isFocused =
              focusedCell?.fixtureId === fixture.id && focusedCell?.columnKey === col.key;
                ...
                >
                  ...
                  {Array.isArray(gelColor) ? (
                    ...
                    <div
                      ...
                      title={`${cellValue} (${gelColor[0]} + ${gelColor[1]})`}
                    >
