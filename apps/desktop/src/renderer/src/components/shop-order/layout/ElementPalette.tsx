...
export function ElementPalette({ onDragStart, onDragEnd }: ElementPaletteProps) {
  const [activeCategory, setActiveCategory] = useState<'all' | 'data' | 'content' | 'visual'>(
    'all',
  );
  const [searchQuery, setSearchQuery] = useState('');
  const [searchExpanded, setSearchExpanded] = useState(false);
  const [collapsedSections, setCollapsedSections] = useState<Set<string>>(
    new Set(['data', 'content', 'visual']),
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {filteredElements.length === 0 ? (
          ...
          <div className="space-y-4">
            {(Object.keys(groupedElements) as Array<keyof typeof groupedElements>).map(
              (category) => {
