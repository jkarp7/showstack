...
export function ReportTableRenderer({
  ...
  const handleReorder = useCallback(
    (fromIndex: number, toIndex: number) => {
      ...
      const adjustedTargetIndex =
        movedColumnFullIndex < targetColumnFullIndex
  ...
  const handleResizeToContent = useCallback(
    (columnId: string) => {
  ...
  const handleConfirmMerge = useCallback(
    (mergedColumnIds: string[], separator: string) => {
  ...
  const handleUnmergeColumn = useCallback(
    (columnId: string) => {
      if (!onColumnsChange) return;
      const column = columns.find((c) => c.id === columnId);
  ...
  return (
    <div
      className="bg-white text-black report-content p-8"
    ...
    >
      {data.groups.map((group, groupIndex) => (
        <div
          ...
          className="mb-8"
          style={
            groupIndex > 0 && organization?.groupPageBreaks
