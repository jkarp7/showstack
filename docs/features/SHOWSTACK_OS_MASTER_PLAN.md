...
_Goal: Stabilize the current codebase to support future complexity._
...
_Goal: Build the shared engines that power all modules._
...
- **Implementation:** A `Connections` table that tracks flow between UUIDs.
  - _Lighting:_ Dimmer -> Fixture
  - _Sound:_ Console -> Amp -> Speaker
  - _Video:_ Server -> Processor -> Wall
