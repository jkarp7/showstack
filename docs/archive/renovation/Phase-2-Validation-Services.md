...
export function bulkInsert(tableName: string, records: unknown[][], columns: string[]): number;
```
...
// After:
throw new DatabaseError('Failed to process database file', 'database:load', false, error); // Generic message, details in context
